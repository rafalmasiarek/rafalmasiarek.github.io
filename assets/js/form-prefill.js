...
(function () {
  ...
  // ---------------------------------------------------------
  // NEW: Contact link builder (prefill via queryPrefix)
  // Expects a button/link: <a id="ask-about-vinyl" ...>
  // Uses queryPrefix "af_" => af_subject, af_message
  // ---------------------------------------------------------
  function buildContactUrlForVinyl(detail) {
    ...
    const subject = `Offer / question: ${artist} – ${title}`;
    const message =
      `Hej\n` +
      `chcialbym zadac pytanie / zlozyc oferte o plyte ${artist} - ${title}\n\n` +
